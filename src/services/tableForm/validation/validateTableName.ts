...
import { tableFormNameErrorMutation } from 'src/store/mutations/tableForm/TableFormNameErrorMutation';

interface State {
  name: string;
}

const selector = (state: AppState): State => ({ name: state.tableForm.name });
